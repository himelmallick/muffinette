...
```{r compare0.05,echo=FALSE,eval=TRUE}
...
fig_muffinette_group <- metafits_muffinette_group %>%
  filter(qval < 0.05) %>%
  mutate(feature = sub("^species:", "", feature)) %>%
  ...
  labs(y = NULL, x = NULL, title = "muffinette_group")

if(length(selected_features_muffinette1) == 0) {
    fig <- ggarrange(fig_mmuphin, fig_muffinette_group, nrow = 2, heights = c(0.75, 1))
    ...
    fig <- ggarrange(fig_mmuphin, fig_muffinette, fig_muffinette_group, nrow = 3, heights = c(0.70, 0.70, 1))
    fig <- annotate_figure(fig,
                       left = text_grob("Differentially Abundant Species",
                                        size = 8, face = "bold", rot = 90),
                       bottom = text_grob("Estimated Coefficients",
...
```{r compare0.25,echo=FALSE,eval=TRUE}
fig_mmuphin <- metafits_mmuphin %>%
...
if(length(selected_features_muffinette2) == 0) {
    fig2 <- ggarrange(fig_mmuphin, fig_muffinette_group, nrow = 2, heights = c(0.75, 1))
    ...
    fig2 <- ggarrange(fig_mmuphin, fig_muffinette, fig_muffinette_group, nrow = 3, heights = c(0.70, 0.70, 1))
